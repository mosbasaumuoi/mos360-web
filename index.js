export default {
  async fetch(request, env) {
    const html = `
    <!DOCTYPE html>
    <html lang="vi">
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>MOS360 - Luyện Thi MOS Chuẩn Quốc Tế</title>
        <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;600;700;800&display=swap" rel="stylesheet">
        <style>
            :root { --primary: #FF5722; --bg: #080808; --card: #121212; --text: #fff; --border: rgba(255,255,255,0.08); }
            * { box-sizing: border-box; margin: 0; padding: 0; }
            body { font-family: 'Plus Jakarta Sans', sans-serif; background: var(--bg); color: var(--text); line-height: 1.6; }

            header { padding: 15px 50px; display: flex; justify-content: space-between; align-items: center; position: sticky; top: 0; background: rgba(8,8,8,0.9); backdrop-filter: blur(10px); z-index: 1000; border-bottom: 1px solid var(--border); }
            .brand { display: flex; align-items: center; gap: 10px; text-decoration: none; color: white; font-weight: 800; font-size: 1.4rem; }
            .brand img { width: 40px; }

            /* SIDE SOCIAL */
            .side-social { position: fixed; right: 20px; top: 50%; transform: translateY(-50%); display: flex; flex-direction: column; gap: 10px; z-index: 9999; }
            .social-item { width: 42px; height: 42px; background: #1a1a1a; border: 1px solid var(--border); border-radius: 50%; display: flex; align-items: center; justify-content: center; text-decoration: none; }
            .social-item img, .social-item svg { width: 20px; height: 20px; }

            .hero { text-align: center; padding: 60px 20px 30px; }
            .hero h1 { font-size: 3.2rem; font-weight: 800; margin-bottom: 10px; }

            .main-container { max-width: 1200px; margin: 0 auto; padding: 0 40px; display: grid; grid-template-columns: 1fr 1fr; gap: 30px; }
            .section-card { background: var(--card); border: 1px solid var(--border); border-radius: 32px; padding: 40px; text-align: center; }

            /* WHEEL LOGIC - CĂN GIỮA NAN */
            .wheel-box { position: relative; width: 300px; height: 300px; margin: 0 auto 25px; }
            .wheel-pointer { position: absolute; top: -15px; left: 50%; transform: translateX(-50%); width: 0; height: 0; border-left: 15px solid transparent; border-right: 15px solid transparent; border-top: 25px solid #FFD700; z-index: 20; }
            .wheel-circle { 
                width: 100%; height: 100%; border-radius: 50%; border: 8px solid #FFD700;
                background: conic-gradient(#ff6b6b 0 90deg, #4ecdc4 90deg 180deg, #ffbe0b 180deg 270deg, #ff006e 270deg 360deg);
                position: relative; animation: spin 20s linear infinite;
            }
            .wheel-btn { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 70px; height: 70px; background: white; border-radius: 50%; z-index: 30; display: flex; align-items: center; justify-content: center; font-weight: 900; color: #000; box-shadow: 0 0 15px rgba(0,0,0,0.5); font-size: 0.8rem; }
            
            .wheel-txt { position: absolute; width: 100%; height: 100%; top: 0; left: 0; }
            .wheel-txt b {
                position: absolute; top: 50%; left: 50%; width: 50%;
                transform-origin: left center; text-align: center;
                padding-left: 50px; /* Đưa chữ vào giữa nan quạt */
                font-size: 0.8rem; font-weight: 900; color: white; text-shadow: 0 2px 4px rgba(0,0,0,0.5);
            }
            .w1 b { transform: rotate(-45deg) translateY(-50%); }
            .w2 b { transform: rotate(-135deg) translateY(-50%); }
            .w3 b { transform: rotate(-225deg) translateY(-50%); }
            .w4 b { transform: rotate(-315deg) translateY(-50%); }

            @keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }

            /* BẢNG VÀNG CAROUSEL */
            .carousel { overflow: hidden; margin-top: 20px; }
            .track { display: flex; gap: 15px; animation: scroll 15s linear infinite; width: max-content; }
            .stu { width: 90px; text-align: center; flex-shrink: 0; }
            .stu img { width: 65px; height: 65px; border-radius: 50%; border: 2px solid #FFD700; margin-bottom: 5px; }
            .stu span { display: block; color: #FFD700; font-size: 0.7rem; font-weight: 800; }
            @keyframes scroll { 0% { transform: translateX(0); } 100% { transform: translateX(-50%); } }

            /* 3 MẢNG CHÍNH - GIỮ NGUYÊN NỘI DUNG */
            .services { max-width: 1200px; margin: 50px auto; padding: 0 40px; display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; }
            .service-item { background: var(--card); padding: 35px; border-radius: 28px; border: 1px solid var(--border); }
            .service-item h3 { font-size: 1.25rem; margin-bottom: 15px; border-left: 3px solid var(--primary); padding-left: 12px; }
            .service-item p { color: #888; font-size: 0.9rem; line-height: 1.7; }

            .btn-gift { background: #E64A19; color: white; border: none; padding: 18px; border-radius: 100px; font-weight: 800; width: 100%; cursor: pointer; text-transform: uppercase; margin-top: 10px; }

            footer { padding: 60px 40px; background: #050505; border-top: 1px solid var(--border); }
            .f-grid { max-width: 1200px; margin: 0 auto; display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 40px; }
            .map-box { border-radius: 20px; overflow: hidden; height: 220px; border: 1px solid var(--border); filter: grayscale(1) invert(0.9); }

            @media (max-width: 900px) { .main-container, .services, .f-grid { grid-template-columns: 1fr; } }
        </style>
    </head>
    <body>

    <header>
        <a href="#" class="brand"><img src="https://github.com/mosbasaumuoi/mos360-web/blob/main/logo%20vien.png?raw=true"> <span>MOS360</span></a>
        <a href="#" style="background:var(--primary); color:white; padding:10px 20px; border-radius:8px; text-decoration:none; font-weight:700;">Đăng nhập</a>
    </header>

    <div class="side-social">
        <a href="#" class="social-item"><img src="https://upload.wikimedia.org/wikipedia/commons/9/91/Icon_of_Zalo.svg"></a>
        <a href="#" class="social-item"><img src="https://upload.wikimedia.org/wikipedia/commons/0/05/Facebook_Logo_%282019%29.png"></a>
        <a href="#" class="social-item"><img src="https://upload.wikimedia.org/wikipedia/commons/b/be/Facebook_Messenger_logo_2020.svg"></a>
        <a href="#" class="social-item"><img src="https://upload.wikimedia.org/wikipedia/commons/0/09/YouTube_full-color_icon_%282017%29.svg"></a>
        <a href="#" class="social-item"><svg viewBox="0 0 24 24" fill="white"><path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.03 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.9-.32-1.98-.23-2.8.12-.91.38-1.57 1.23-1.73 2.19-.11.64-.03 1.3.18 1.91.43 1.13 1.53 1.95 2.73 2.11 1.19.16 2.45-.19 3.33-1.01.62-.57.97-1.38 1-2.22.04-4.52-.01-9.04.01-13.56z"/></svg></a>
    </div>

    <section class="hero">
        <h1>Chuẩn đầu ra cho sinh viên</h1>
        <p style="color:#666;">Luyện thi MOS 1000/1000 - Đồng hành thực chiến trọn đời</p>
    </section>

    <div class="main-container">
        <div class="section-card">
            <h3 style="color:var(--primary); margin-bottom:20px;">🎡 Vòng Quay May Mắn</h3>
            <div class="wheel-box">
                <div class="wheel-pointer"></div>
                <div class="wheel-circle">
                    <div class="wheel-txt w1"><b>GIẢM 50K</b></div>
                    <div class="wheel-txt w2"><b>GIẢM 50%</b></div>
                    <div class="wheel-txt w3"><b>GIẢM 100K</b></div>
                    <div class="wheel-txt w4"><b>GIẢM 80%</b></div>
                </div>
                <div class="wheel-btn">QUAY</div>
            </div>
            <button class="btn-gift">NHẬN QUÀ NGAY</button>
        </div>

        <div class="section-card" style="text-align:left;">
            <h3>🏆 Bảng Vàng MOS360</h3>
            <p style="color:#666; font-size:0.9rem; margin-bottom:15px;">Học viên xuất sắc đạt điểm tối đa tại Hải Phòng.</p>
            <div class="carousel">
                <div class="track">
                    <div class="stu"><img src="https://via.placeholder.com/80"><span>1000/1000</span></div>
                    <div class="stu"><img src="https://via.placeholder.com/80"><span>1000/1000</span></div>
                    <div class="stu"><img src="https://via.placeholder.com/80"><span>980/1000</span></div>
                    <div class="stu"><img src="https://via.placeholder.com/80"><span>1000/1000</span></div>
                    <div class="stu"><img src="https://via.placeholder.com/80"><span>1000/1000</span></div>
                    <div class="stu"><img src="https://via.placeholder.com/80"><span>1000/1000</span></div>
                </div>
            </div>
            <div style="margin-top:30px; border-top:1px solid var(--border); padding-top:20px;">
                <p style="color:var(--primary); font-weight:800;">🎁 ƯU ĐÃI NHÓM:</p>
                <p style="color:#555; font-size:0.85rem;">Đăng ký nhóm 3 người giảm thêm 100k/bạn.</p>
            </div>
        </div>
    </div>

    <div class="services">
        <div class="service-item">
            <h3>Thi Thật 100%</h3>
            <p>Thực hành trực tiếp trên phần mềm mô phỏng sát 100% đề thi quốc tế. Tự tin tuyệt đối khi bước vào phòng thi thật.</p>
        </div>
        <div class="service-item">
            <h3>AI Assistant 24/7 ✨</h3>
            <p>Hệ thống trí tuệ nhân tạo hỗ trợ tìm kiếm tài liệu thông minh và giải đáp mọi thắc mắc về kiến thức tin học ngay lập tức.</p>
        </div>
        <div class="service-item">
            <h3>Đồng Hành Trọn Đời</h3>
            <p>Cung cấp tài liệu và hỗ trợ kỹ năng chuyên sâu, giúp bạn làm chủ mọi luận văn và tác vụ văn phòng thực tế.</p>
        </div>
    </div>

    <footer>
        <div class="f-grid">
            <div><h2 style="color:var(--primary)">MOS360.VN</h2><p style="color:#555">Hotline: 0912.888.360</p></div>
            <div><p style="color:#888">57 Lê Văn Thuyết A, An Biên, Hải Phòng</p></div>
            <div class="map-box"><iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3728.384666504514!2d106.6792345!3d20.8566838!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x314a7af276706915%3A0x676067f967f9c7f9!2zNTcgTMOqIFbEg24gVGh1eeG6v3QgQSwgQW4gQmnDqm4sIEzDqiBDaMOibiwgSOG6o2kgUGjDsm5n!5e0!3m2!1svi!2s!4v1710000000000" width="100%" height="100%" style="border:0;" allowfullscreen="" loading="lazy"></iframe></div>
        </div>
    </footer>

    </body>
    </html>`;
    return new Response(html, { headers: { "Content-Type": "text/html;charset=UTF-8" } });
  }
};
